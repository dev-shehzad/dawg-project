...
import NextPatriotamat from './NextPatriotamat/NextPatriotamat';
...
const Patriotamat = () => {
  ...
  return (
    <div className='w-full'>
      {news.map((item)=>{
         return(
         <div key={item.id} className='w-full max-w-[1100px] mx-auto flex flex-col items-center'>
          <div className='w-full relative overflow-hidden cursor-pointer'>
          <img src={item.img} className='w-full mt-[8%] transform hover:scale-[1.05] hover:opacity-[0.9] transition-[0.1s] ease'/>
          </div>
                <div className='text-[16px] font-[400] text-gray-700 font-playfair pt-5 object-cover'>{item.date}</div>
                <div className='text-[48px] text-gray-800 max-md:text-[40px] max-sm:text-[35px] font-[400] font-playfair hover:opacity-[0.5] cursor-pointer transition-[0.1s] ease-in-out'>Patriotamat Locakzp</div>
                <div className='text-[17px] max-md:text-[14px] font-[400] font-sans py-3 text-gray-600 text-center'>{item.des}</div>
      ...
      {Patriotamat && (
        <Link To ={Patriotamat}>
          <h2 className="px-4 py-2 bg-blue-500 text-white rounded">Previous</h2>
        </Link>
      )}
      {NextPatriotamat && (
        <Link To ={NextPatriotamat}>
          <h2 className="px-4 py-2 bg-blue-500 text-white rounded">Next</h2>
        </Link>
      )}
    </div>
